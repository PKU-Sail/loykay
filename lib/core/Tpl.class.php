...
class libTpl {
    	...
    	if(ErrorHandler::hasError() && DEBUG_MODE == 1)
       	{
			...
			ob_start();
        	extract($this->var);
        	include $this->tpl['complied_filename'];
    ...
    public function complie($tpl_name) {
        ...
        if (!is_writeable(ROOT_PATH . '/' . $this->tpl_cache)) {
    	...
    	$contents_nonmode = file_get_contents($filename);		
		$module_regexp = array(
		'mod' => '(\{module=".*"\})',
		);
		...
		while($module_code = array_shift($template_code_mode))
		{
			...
			{
				$contents .= file_get_contents(ROOT_PATH . '/tpl/models/' . $module_c[1] . '.html');
			}
			else
			{
        ...
        //end complie
        //save file
        file_put_contents($cachefilename, $complied_code);
    }
