...
class ProxyAction
{
	...
	public function __call($name , $arguments)
	{
		...
		if(method_exists($this->target, 'preCall'))
		{
			call_user_func(array($this->target,'preCall'),$this->pre_arguments);
		}
		call_user_func(array($this->target,$name));
		if(method_exists($this->target, 'lasCall'))
		{
			call_user_func(array($this->target,'lasCall'),$this->las_arguments);
		}
	}
}